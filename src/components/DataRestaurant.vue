<template lang="html">
  ...
  <div class="card">
    <DataTable :value="tables" tableStyle="min-width: 50rem">
      <Column field="identification" header="Mesa" bodyStyle="text-align:center"></Column>
      <Column field="capacity" header="Capacidade" bodyStyle="text-align:center"></Column>
      <Column field="status" header="Status" bodyStyle="text-align:center"></Column>
      ...
      <Column header="Reservar" bodyStyle="text-align:center">
        <template #body="row">
          <Button v-if="row.data.status == 0" severity="success" icon="pi pi-calendar-clock" @click="reserve(row.data)" />
          <Button v-if="row.data.status == 2" severity="danger" icon="pi pi-calendar-clock" @click="cancelReserve(row.data.id)" />
        </template>
      </Column>
    </DataTable>
  </div>
  ...
  <Dialog v-model:visible="visibleEdit" modal :header="dialogEditHeader"
    :style="{ width: '25rem' }">
      <div class="row">
    ...
    <div class="row m-2 justify-content-end">
      <div class="col-3">
        <Button type="button" label="Excluir" severity="danger" @click="visibleEdit = false" raised rounded size="small" ></Button>
      </div>
      <div class="col-3">
        <Button type="button" label="Cancelar" severity="secondary" @click="visibleEdit = false" raised rounded size="small"></Button>
      </div>
      <div class="col-3">
  ...
  <Dialog v-model:visible="visibleReserve" modal :header="dialogReserveHeader"
    :style="{ width: '25rem' }">
      <div class="row">
    ...
    <div class="row m-2 justify-content-end">
      <div class="col-3">
        <Button type="button" label="Cancelar" severity="secondary" @click="visibleReserve = false" raised rounded size="small"></Button>
      </div>
      <div class="col-3">
        <Button type="button" label="Salvar" @click="confirmReserve()" raised rounded size="small"></Button>
      </div>
    </div>
  </Dialog>
...
const confirmEdit = () =>{
  ...
  showToast('success', "Sucesso", 'Mesa salva com sucesso!')
}
...
const confirmReserve = () => {
  const tableId = tableEdit.value.id;
  const datetime = datetime24h.value;
  const email = customerEmail.value;

  ReservationService.Create(tableId, datetime, email)
  .then(response => {
    showToast('success', "Sucesso", "A reserva já está sendo preparada!")
